...
- **免费测试分发（未公证）**：当前产物为 ad-hoc 签名（`Sign to Run Locally`），未 Developer ID 签名、未 notarize。这是免费分发方案的已知限制（不是 bug）：DMG 接收者首次启动需在「系统设置 → 隐私与安全性」点击「仍要打开」；不要使用 `sudo spctl --master-disable`。需要无提示公开分发时需另行完成 Developer ID + notarization。
