...
Stage 11 实际界面正式启用 Sidebar“穿搭”，主页面采用原生 toolbar + 自适应缩略图网格 + 详情栏。Toolbar 提供名称/备注搜索、归档范围、仅收藏与最近修改/最新创建/名称/收藏优先排序；详情支持编辑、收藏、归档/恢复、永久删除确认和“在 AI 试衣间中打开”。无数据与筛选无结果使用不同 empty state。载入有问题项时显示数量摘要并要求确认；无可载入项时保留当前 Try-On session。列表仅解码 thumbnail，缺失位置使用系统 placeholder，不创建合成封面文件。
